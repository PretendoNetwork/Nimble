...
typedef enum VPADTPResolution { VPAD_TP_1920x1080, VPAD_TP_1280x720, VPAD_TP_854x480 } VPADTPResolution;
...
#define OS_THREAD_ATTRIB_AFFINITY_ANY \
    OS_THREAD_ATTRIB_AFFINITY_CPU0 | OS_THREAD_ATTRIB_AFFINITY_CPU1 | OS_THREAD_ATTRIB_AFFINITY_CPU2
...
extern int (*FSOpenFile)(FSClient* client, FSCmdBlock* block, const char* path, const char* mode, int* handle,
                         uint32_t flags);
...
extern int (*FSReadDir)(FSClient* client, FSCmdBlock* block, int dirHandle, FSDirEntry* returnedDirEntry,
                        int errHandling);
...
extern int (*FSWriteFile)(FSClient* client, FSCmdBlock* block, uint8_t* buffer, uint32_t size, size_t count, int handle,
                          uint32_t unk1, uint32_t flags);
extern int (*FSReadFile)(FSClient* client, FSCmdBlock* block, uint8_t* buffer, uint32_t count, size_t size, int handle,
                         uint32_t unk1, uint32_t flags);
extern int (*FSReadFileWithPos)(FSClient* client, FSCmdBlock* block, uint8_t* buffer, uint32_t count, size_t size,
...
extern int (*FSMount)(FSClient* client, FSCmdBlock* cmd, char* source, const char* target, uint32_t bytes,
                      uint32_t flags);
...
extern bool (*OSCreateThread)(OSThread* thread, void* entry, int argc, char* argv, void* stack, uint32_t stackSize,
...
extern int (*SAVEOpenFile)(FSClient* client, FSCmdBlock* block, uint8_t accountSlotNo, const char* path,
                           const char* mode, int* fileHandle, int errHandling);
